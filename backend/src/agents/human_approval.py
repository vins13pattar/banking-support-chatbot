...
def create_approval_request(state: dict) -> dict:
    """Helper to format the approval request for the frontend."""
    return {
        "approval_id": state.get("thread_id"), # Using thread_id as approval ID for simplicity
...
async def human_approval_node(state: dict) -> dict:
    ...
    approval_request = create_approval_request(state)
    ...
    # If rejected, clear the proposed action and go back to supervisor
    if not approved:
        ...
        return {
            ...
            "active_agent": None # back to supervisor
        }
